...
tf.flags.DEFINE_string("logs_dir", "logs_vggf/", "path to logs directory")
...
'''
        shape = tf.shape(image)
        deconv_shape3 = tf.pack([shape[0], shape[1], shape[2], NUM_OF_CLASSESS])
        W_t3 = utils.weight_variable([64, 64, NUM_OF_CLASSESS, NUM_OF_CLASSESS], name="W_t3")
        b_t3 = utils.bias_variable([NUM_OF_CLASSESS], name="b_t3")
        conv_t3 = utils.conv2d_transpose_strided(conv8, W_t3, b_t3, output_shape=deconv_shape3, stride=32)
...
def main(argv=None):
    ...
    if FLAGS.mode == "train":
        ...
        for i in xrange(200/FLAGS.batch_size):
            valid_images, valid_annotations = validation_dataset_reader.next_batch(FLAGS.batch_size)
            ...
            for itr in range(FLAGS.batch_size):
                utils.save_image(valid_images[itr].astype(np.uint8), FLAGS.logs_dir, name="inp_" + str(1+itr+FLAGS.batch_size*i))
                utils.save_image(valid_annotations[itr].astype(np.uint8), FLAGS.logs_dir, name="gt_" + str(1+itr+FLAGS.batch_size*i))
                utils.save_image(pred[itr].astype(np.uint8), FLAGS.logs_dir, name="pred_" + str(1+itr+FLAGS.batch_size*i))
