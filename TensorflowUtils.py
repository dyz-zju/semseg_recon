...
def conv2d_basic_unpadded(x, W, bias):
    conv = tf.nn.conv2d(x, W, strides=[1, 1, 1, 1], padding="VALID")
    return tf.nn.bias_add(conv, bias)


def conv2d_strided_unpadded(x, W, b):
    conv = tf.nn.conv2d(x, W, strides=[1, 4, 4, 1], padding="VALID")
    return tf.nn.bias_add(conv, b)
...
def lr_normalisation(x,p):
    return tf.nn.local_response_normalization(x, depth_radius = p[0], bias = p[1], alpha = 0.0001, beta = 0.75)
