...
tf.flags.DEFINE_string("logs_dir", "logs_vgg_my3/", "path to logs directory")
...
def inference(image, keep_prob):
    ...
    with tf.variable_scope("inference"):
        ...
        deconv_shape3a = tf.pack([shape[0], shape[1], shape[2], NUM_OF_CLASSESS])
        W_t3a = utils.weight_variable([16, 16, NUM_OF_CLASSESS, deconv_shape2[3].value], name="W_t3a")
        b_t3a = utils.bias_variable([NUM_OF_CLASSESS], name="b_t3a")
        conv_t3a = utils.conv2d_transpose_strided_valid(fuse_2, W_t3a, b_t3a, output_shape=deconv_shape3a, stride=8)
        annotation_pred = tf.argmax(conv_t3a, dimension=3, name="prediction")

        deconv_shape3b = image_net["conv1"].get_shape()
        W_t3b = utils.weight_variable([3, 3, deconv_shape3b[3].value, deconv_shape2[3].value], name="W_t3b")
        b_t3b = utils.bias_variable([deconv_shape3b[3].value], name="b_t3b")
        conv_t3b = utils.conv2d_transpose_strided(fuse_2, W_t3b, b_t3b, output_shape=tf.shape(image_net["conv1"]),stride=2)

        shape = tf.shape(image)
        deconv_shape4 = tf.pack([shape[0], shape[1], shape[2], 3])
        W_t4 = utils.weight_variable([11, 11, 3, deconv_shape3b[3].value], name="W_t4")
        b_t4 = utils.bias_variable([3], name="b_t4")
        conv_t4 = utils.conv2d_transpose_strided_valid(conv_t3b, W_t4, b_t4, output_shape=deconv_shape4, stride=4)

    return tf.expand_dims(annotation_pred, dim=3), conv_t3a, conv_t4+mean_pixel
#######################################################
'''
        shape = tf.shape(image)
...
def main(argv=None):
    ...
    pred_annotation, logits, reconstruct = inference(image, keep_probability)
    ...
    loss1 = tf.reduce_mean((tf.nn.sparse_softmax_cross_entropy_with_logits(logits,
                                                                          tf.squeeze(annotation, squeeze_dims=[3]),
                                                                          name="entropy")))


    loss2 = tf.div(tf.reduce_mean(tf.square(image -  reconstruct)), 2, name="mse")
    #loss2 = tf.nn.l2_loss(image-reconstruct)
    tf.scalar_summary("entropy", loss1)
    loss = loss2+500*loss1
    ...
    if FLAGS.mode == "train":
        ...
        for i in xrange(200/bs):
            ...
            for itr in range(FLAGS.batch_size):
                utils.save_image(valid_images[itr].astype(np.uint8), FLAGS.logs_dir, name="inp_" + str(1+itr+i*bs))
                utils.save_image(valid_annotations[itr].astype(np.uint8), FLAGS.logs_dir, name="gt_" + str(1+itr+i*bs))
                utils.save_image(pred[itr].astype(np.uint8), FLAGS.logs_dir, name="pred_" + str(1+itr+i*bs))
